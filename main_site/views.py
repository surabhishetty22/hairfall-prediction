...
def Home(request):
    if request.method == 'GET':
        try:
            cur_user = request.user  # Use the request.user attribute to get the current user
            cur_user_pro = DrProfile.objects.get(user=cur_user)  # Assuming DrProfile has a ForeignKey to User
            return render(request, 'main_site/home.html', {"user": cur_user, "profile": cur_user_pro})
        except DrProfile.DoesNotExist:
            return HttpResponse('Forbidden')
    else:
        return HttpResponse('Forbidden')
...
def Login(request):
    if request.method == 'GET':
        ...
        username = request.POST.get('username')  # Change 'lic_num' to 'username'
